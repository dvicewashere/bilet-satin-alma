...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($user) {
            // Hata ayıklama: Şifre doğrulamasını kontrol et
            ...
            if ($password_verified) {
                ...
                $error = 'Şifre hatalı. Kullanıcı bulundu ama şifre yanlış.';
            }
        } else {
...
<body style="background: #181818; min-height: 100vh; position: relative; overflow-x: hidden;">
    <div class="auth-container" style="min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 2rem; background: #181818;">
        <div class="auth-form" style="background: rgba(255, 255, 255, 0.98); border-radius: 25px; padding: 2.5rem 2rem 2rem; box-shadow: 0 20px 40px rgba(0, 0, 0, 0.3); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.2); width: 100%; max-width: 520px; position: relative; overflow: hidden;">
        ...
        <svg class="mySVG" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 200 200" style="width:100%;height:100%">
          ...
          <g class="arms" clip-path="url(#armMask)">
            ...
            <div class="auth-header" style="text-align: center; margin-bottom: 1.25rem;">
                <h2 style="color: #181818; margin-bottom: 0.25rem; font-size: 1.8rem; font-weight: 800; letter-spacing: .3px;">Giriş Yap</h2>
                <p style="color: #666; font-size: .95rem;">Hesabınıza giriş yapın</p>
            ...
            <form method="POST" action="">
                <div class="form-group inputGroup inputGroup1" style="margin-bottom: 1.2rem; position: relative;">
                    <label for="email" style="color: #181818; font-weight: 600; margin-bottom: 0.5rem; display: block;">E-posta</label>
                    <input type="text" id="email" name="email" required 
                           value="<?php echo isset($_POST['email']) ? htmlspecialchars($_POST['email']) : ''; ?>" 
                           style="width: 100%; padding: 1rem; border: 2px solid #f1c2c6; border-radius: 14px; font-size: 1rem; background: #fff; color: #181818; transition: all 0.2s ease; box-shadow: 0 0 0 rgba(0,0,0,0);">
                    <p class="helper helper1" style="position:absolute;left:0;top:0;transform:translate(1.4em,2.6em) scale(1);transform-origin:0 0;color:#a01a2e;font-size:1em;opacity:.65;pointer-events:none;"></p>
                </div>
                
                <div class="form-group inputGroup inputGroup2" style="margin-bottom: 1.2rem;">
                    <label for="password" style="color: #181818; font-weight: 600; margin-bottom: 0.5rem; display: block;">Şifre</label>
                    <input type="password" id="password" name="password" required 
                           style="width: 100%; padding: 1rem; border: 2px solid #f1c2c6; border-radius: 14px; font-size: 1rem; background: #fff; color: #181818; transition: all 0.2s ease;">
                </div>
                
                <button type="submit" class="btn btn-primary btn-full" style="width: 100%; padding: 1.1rem; background: linear-gradient(135deg, #e22027 0%, #c41e3a 100%); color: #ffffff; text-decoration: none; border-radius: 18px; font-weight: 700; text-align: center; border: none; cursor: pointer; transition: transform .08s ease; box-shadow: 0 4px 14px rgba(226, 32, 39, 0.3); font-size: 1.05rem;">
            ...
            <div class="auth-footer" style="text-align: center; margin-top: 2rem;">
                <p style="color: #666; margin-bottom: 0.5rem;">Hesabınız yok mu? <a href="register.php" style="color: #e22027; text-decoration: none; font-weight: 600;">Kayıt olun</a></p>
                <p style="color: #666;"><a href="index.php" style="color: #e22027; text-decoration: none; font-weight: 600;">Ana sayfaya dön</a></p>
            </div>
            
            <!-- Test Hesapları -->
            <div class="test-accounts" style="margin-top: 1.5rem; padding: 1.25rem; background: rgba(226, 32, 39, 0.08); border-radius: 15px; border: 2px solid #e22027;">
                <h4 style="color: #181818; font-size: 1.1rem; font-weight: bold; margin-bottom: 1rem; text-align: center;">Test Hesapları:</h4>
                <div class="test-account" style="margin-bottom: 0.5rem; color: #181818;"><strong style="color: #e22027;">Admin:</strong> admin@dvice.com / admin123</div>
                <div class="test-account" style="margin-bottom: 0.5rem; color: #181818;"><strong style="color: #e22027;">Yolcu:</strong> yolcu@dvice.com / test123</div>
                <div class="test-account" style="color: #181818;"><strong style="color: #e22027;">Firma (Metro):</strong> metro@dvice.com / deneme123</div>
                </div>
            </div>
        </div>
    ...
    (function(){
        ...
        function onEmailInput(){
            if(!emailEl) return;
            var svgPos = getPosition(svg);
            var caretX = svgPos.x + svg.offsetWidth/2;
            var caretY = svgPos.y + svg.offsetHeight/2;
            var dX = (Math.min(emailEl.selectionEnd||0, (emailEl.value||'').length) / Math.max((emailEl.value||'').length||1,1)) - 0.5;
            var moveX = dX * 10;
            TweenMax.to(eyeL, 0.4, {x: moveX, ease: Power2.easeOut});
            TweenMax.to(eyeR, 0.4, {x: moveX, ease: Power2.easeOut});
        }
        if(emailEl){
            emailEl.addEventListener('input', onEmailInput);
            emailEl.addEventListener('focus', function(){
                var g = emailEl.parentElement; if(g){ g.classList.add('focusWithText'); }
            });
            emailEl.addEventListener('blur', function(){
                var g = emailEl.parentElement; if(g){ g.classList.remove('focusWithText'); }
                TweenMax.to([eyeL,eyeR], 0.4, {x:0, y:0});
            });
        }
    })();
    </script>
    ...
    (function(){
      ...
      function coverEyes(){ safeTo(armL, .45, {x: -93, y: 2, rotation: 0, ease: Quad.easeOut}); safeTo(armR, .45, {x: -93, y: 2, rotation: 0, ease: Quad.easeOut, delay: .1}); }
      function uncoverEyes(){ safeTo(armL, 1.35, {y: 220, ease: Quad.easeOut}); safeTo(armL, 1.35, {rotation: 105, ease: Quad.easeOut, delay: .1}); safeTo(armR, 1.35, {y: 220, ease: Quad.easeOut}); safeTo(armR, 1.35, {rotation: -105, ease: Quad.easeOut, delay: .1}); }
      ...
      if(password){ password.addEventListener('focus', coverEyes); password.addEventListener('blur', uncoverEyes); }
